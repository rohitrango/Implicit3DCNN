...
if __name__ == '__main__':
    ...
    embed = encoder.embeddings[None] * 100000  # [1, N, 2]
    embed = embed.expand(batch, -1, -1).contiguous()
    resolutions = encoder.resolutions
    offsets = encoder.offsets
    
    # define layer with zero bias
    layer = AbstractConv3D(2, 16, resolutions, offsets, 3, bias=True, num_levels=16, log_hashmap_size=L).cuda()
    ...
    for i in range(encoder.num_levels):
        ...
        # get conv output
        conv_out = F.conv3d(embed_lvl, conv_lvl, bias=None, stride=1, padding=1).permute(0, 4, 3, 2, 1).contiguous()  # [1, r, r, r, 4]
