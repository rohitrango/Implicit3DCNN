...
import os
from networks.unet_refine import UNet3DEncoder
...
if __name__ == '__main__':
    ...
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=1, shuffle=True, num_workers=args.num_workers)
    ...
    encoder = UNet3DEncoder(inp_channels).cuda()
    decoder_size = sum(encoder.f_maps) * 3
    # load decoder
    decoder = nn.Sequential(
        nn.Linear(decoder_size, 256),
        nn.GELU(),
        nn.Linear(256, 256),
        nn.GELU(),
        nn.Linear(256, 256),
    ...
    optim = torch.optim.AdamW(encoder.parameters(), lr=3e-4)
    ...
    for epoch in range(args.num_epochs):
        ...
        for datum in pbar:
            ...
            coord_xy, coord_yz, coord_xz = torch.stack([x, y], dim=-1), torch.stack([y, z], dim=-1), torch.stack([x, z], dim=-1)
            coord_xy, coord_yz, coord_xz = coord_xy.unsqueeze(1), coord_yz.unsqueeze(1), coord_xz.unsqueeze(1)
            ## encoded features
            enc_fxy = [F.grid_sample(f, coord_xy, align_corners=True) for f in f_xy]  # [B, C, 1, num_points]
            enc_fyz = [F.grid_sample(f, coord_yz, align_corners=True) for f in f_yz] # [B, C, 1, num_points]
            enc_fzx = [F.grid_sample(f, coord_xz, align_corners=True) for f in f_xz]
            enc = torch.cat(enc_fxy + enc_fyz + enc_fzx, dim=1)  # [B, C, 1, num_points]
            enc = enc.squeeze(2).permute(0, 2, 1)  # [B, num_points, C]
            pred_img = decoder(enc).permute(0, 2, 1)  # [B, C, num_points]
            # get ground truth points
            gt_points = F.grid_sample(img, torch.stack([x, y, z], dim=-1)[:, None, None], align_corners=True)  # [B, C, 1, 1, num_points]
            gt_points = gt_points.squeeze(2).squeeze(2)  # [B, out, num_points]
            loss = F.mse_loss(pred_img, gt_points)
            ...
            l1_loss = F.l1_loss(pred_img, gt_points)
            # step
            (loss + 0.1*l1_loss).backward()
            pbar.set_description("Epoch: {}/{}, Loss: {:06f}, L1 loss: {:06f}, PSNR: {:06f}".format(epoch, args.num_epochs, loss.item(), l1_loss.item(), psnr))
            optim.step()
            optim_d.step()
            # append to list
        ...
        if np.mean(psnr_thisepoch) > best_psnr:
            ...
            torch.save(_to_cpu(encoder.state_dict()), osp.join(args.output_dir, "unet_encoder{}.pth".format(suffix)))
            torch.save(_to_cpu(decoder.state_dict()), osp.join(args.output_dir, "unet_decoder{}.pth".format(suffix)))
